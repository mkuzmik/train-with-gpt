# Strava API Policy: what it means for this server (research)

Companion to [`auth-without-strava.md`](auth-without-strava.md), which turns
this research into a design. Moved here from the "more training data"
proposal (PR #12), which now only covers intervals.icu data.

The Strava API Policy that took effect on 2026-06-01 restricts using Strava
data with AI applications and MCP servers. This doc quotes the relevant text
and compares the ways the server could get training data. The login question
and the decisions for the owner live in `auth-without-strava.md`.
...
- Hosted login therefore has to **move off Strava OAuth.** The replacement
  login, the migration of existing users and the rollout are designed in
  [`auth-without-strava.md`](auth-without-strava.md).
...
- Stdio already uses intervals.icu, and every new tool in
  `more-training-data.md` (PR #12) is intervals.icu-first.
...
Login options (compared with the non-provider logins in
`auth-without-strava.md`):
...
4. **Tool overlap.** Strava's `get_athlete_profile`, `list_activities` and
   ...
   (see `more-training-data.md` in PR #12).
...
A login that belongs to no data provider (email, Google, GitHub, passkeys).
Compared in depth, with a recommendation, in
[`auth-without-strava.md`](auth-without-strava.md#3-login-options-compared).

## Recommendation (data sources)

- **Keep the freeze:** no new Strava processing, storage or caching. Build
  every new data feature on intervals.icu (option 2).
- **Strava data comes from the official Strava MCP** (option 5), added by the
  user as a separate connector. This server never calls it.
- **Retire this server's Strava app** (options 1 and 6), including the Strava
  login. How, in which order and with which replacement login is the subject
  of [`auth-without-strava.md`](auth-without-strava.md).
- Don't integrate device APIs (option 3) or file uploads (option 4) unless
  intervals.icu stops being viable.

## Questions for developers@strava.com

Describe the server (open-source, hobby, Standard tier) and ask:
1. Whether the current hosted Strava path may continue, and if not, what
   grace period applies.
2. Whether Strava OAuth *only for login* is permitted in an MCP/AI product.
3. Whether notes Claude wrote from Strava data (in a repo the operator
   controls) must be deleted under §7.4 when the app stops using the API.
4. Whether a user's Claude passing Strava MCP output into a third-party MCP
   server's notes tool is acceptable.
5. Whether the Strava athlete id, kept as an internal key or left in the
   training repo's history after a rename, counts as "Strava Data" that §7.4
   requires deleting.
